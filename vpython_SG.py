...
def dados_simul():
    ...
    for i in range(int((tf - t) / dt)):
        ...
        # Debug prints
        if i % 100 == 0:  # Imprimir a cada 100 iterações
            print(f"Tempo: {t:.2f}, x: {x.flatten()}, u: {u.flatten()}, x_est: {x_est.flatten()}")
...
def inicializa(T):
    tam_eixo, tam_cubo, tam_mola, esp_chao = 2, 2, 5, 0.05
    ...
    mola1 = helix(pos=vector(0, tam_cubo, 0), thickness=0.2, color=color.purple)
    mola2 = helix(pos=vector(0, tam_cubo, 0), thickness=0.2, color=color.purple)
    mola3 = helix(pos=vector(0, tam_cubo, 0), thickness=0.2, color=color.purple)
    
    massa1 = box(opacity=0.5, size=2 * tam_cubo * vec(1, 1, 1), color=color.green)
    massa2 = box(opacity=0.5, size=2 * tam_cubo * vec(1, 1, 1), color=color.red)
    massa3 = box(opacity=0.5, size=2 * tam_cubo * vec(1, 1, 1), color=color.blue)
...
def move(X, T, f1, f2):
    ...
    disp_rate = 20  # Ajuste conforme necessário

    for i in range(len(T)):
        rate(disp_rate)
        x1 = X[0][i] + tam_mola
        x2 = X[2][i] + 2 * (tam_mola + tam_cubo)
        x3 = X[4][i] + 2 * (tam_mola + tam_cubo)
        v1, v2, v3 = X[1][i], X[3][i], X[5][i]
        
        mola1.axis.x = x1
        massa1.pos.x = x1 + tam_cubo
        mola2.pos.x, mola2.axis.x = x1 + 2 * tam_cubo, x2 - x1 - 2 * tam_cubo
        massa2.pos.x = x2 + tam_cubo
        mola3.pos.x, mola3.axis.x = x2 + 2 * tam_cubo, x3 - x2 - 2 * tam_cubo
        massa3.pos.x = x3 + tam_cubo
        
        forca1.pos.x, forca1.axis.x = x1 + delta_f(f1[i]), f1[i] / 2
        forca2.pos.x, forca2.axis.x = x2 + delta_f(f2[i]), f2[i] / 2
...
imprime(T, X, f1, f2)
forca1, forca2, forca3, mola1, mola2, mola3, massa1, massa2, massa3, gx1, gx2, gf1, gf2, gv1, gv2, tam_cubo, tam_mola = inicializa(T)
sleep(3)
move(X, T, f1, f2)
